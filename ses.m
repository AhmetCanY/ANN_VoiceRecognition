function sesTanima = ses(uzanti)
s = load('egitimKumesi');

ADS2= audioDatastore(uzanti,'IncludeSubfolders',1);
ADS2.Labels = extractBetween(ADS2.Files,fullfile(uzanti,filesep),filesep);

Fs = s.Fs;

frameDuration = 200e-3;
overlapDuration = 40e-3;
frameLength = floor(s.Fs*frameDuration); 
overlapLength = round(s.Fs*overlapDuration);

[XTest1,YTest1]=preprocessAudioData(ADS2,frameLength,overlapLength,s.Fs);
YPred1 = classify(s.net,XTest1);
a = countcats(YPred1);
b = a(1);
for i = 1:5
    if a(i) >= b
...
if c == 5
    sesTanima = ('Test edilen ses sizin (Ahmet Can) olduğu doğrulanmıştır.');
else
    sesTanima = ('Test edilen ses sizin sesiniz değildir.');
end 
